...
public class SalesforceConn {
    ...
    */
   /*
    * public void getAttribList() { new GetAttribListTask(context).execute(); }
    */
   /*
    * protected class GetAttribListTask extends BaseAsyncTask { private String
    * responseString = null; private final String TAG =
    * GetAppListTask.class.getSimpleName();
    * 
    * public GetAttribListTask(Context context) { super(context); }
    * 
    * @Override protected String doInBackground(Void... params) { Log.i(TAG,
    * "Sending GET request to get app list");
    * 
    * String accessToken = context.getSharedPreferences(Keys.PREFS_NS, 0)
    * .getString(Keys.ACCESS_TOKEN, null); String instanceUrl =
    * context.getSharedPreferences(Keys.PREFS_NS, 0)
    * .getString(Keys.INSTANCE_URL, null);
    * 
    * if (accessToken == null) { Log.e(TAG, "No access token currently saved");
    * return null; }
    * 
    * // Put together the HTTP Request to be sent to Salesforce for the //
    * Attibute list HttpPost attribRequest = new HttpPost(String.format(
    * SALESFORCE_BASE_REST_URL, instanceUrl, ATTRIBS_URL_SUFFIX));
    * attribRequest.setHeader("Authorization", "Bearer " + accessToken);
    * 
    * // Add AppLabel parameter String appLabel =
    * context.getSharedPreferences(Keys.PREFS_NS, 0) .getString(Keys.APP_LABEL,
    * null); List<NameValuePair> nameValuePairs = new
    * ArrayList<NameValuePair>(); nameValuePairs.add(new
    * BasicNameValuePair("appLabel", appLabel));
    * 
    * try { attribRequest.setEntity(new UrlEncodedFormEntity(nameValuePairs));
    * // Get the response string, the Attribute List in JSON form responseString
    * = EntityUtils.toString(client.execute(attribRequest) .getEntity()); }
    * catch (ClientProtocolException e) { Log.e(TAG, e.getMessage()); } catch
    ...
    * 
    * if (responseString == null) { final String errorMessage =
    * "ERROR: Attempted to parse null App list."; Toast.makeText(context,
    * errorMessage, Toast.LENGTH_SHORT).show(); Log.e(TAG, errorMessage);
    * return; }
    * 
    * // Try to parse the resulting JSON List<EventDescription> eventList =
    * null; try { eventList = EventDescription.parseList(responseString);
    * 
    * TempoDatabase tempoDatabase = TempoDatabase.getInstance();
    * tempoDatabase.setEventDescriptions(eventList); } catch (JSONException e) {
